...
`##` 見出しを1つも含まない場合は、各非空行を1プロンプトとして読み込む。先頭や途中の空行、および `#` から `######` までのMarkdown見出しは無視される。
...
この例は3プロンプトとして読み込まれる。ファイル内に有効な `##` 見出しが1つでもある場合は、従来どおり `##` 区切り形式が優先される。
